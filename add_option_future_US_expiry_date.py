...
# 옵션만기 일정은 category='일반' + isCaution=True 로 저장('주의'는 카테고리가 아니라 플래그).
CATEGORY_NAME = "일반"
...
def get_third_friday(year: int, month: int) -> int:
    # calendar.FRIDAY == 4. monthcalendar()의 각 주(week)에서 금요일 칸만 모아 3번째 값을 취함.
...
def run_option_expiry_crawler():
    ...
    print("\n" + "=" * 60)
    print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] 🚀 미국 옵션만기일(매월 3번째 금요일) 일정 생성기 가동")
    print("=" * 60)
    ...
    try:
        ...
        for year, month in target_months:
            day = get_third_friday(year, month)
            ...
            if len(existing_docs) > 0:
                ...
                update_count += 1
                print(f"🔄  [정보 업데이트] 날짜: {db_date_str} | 세부내용/주의표시를 갱신했습니다.")
            else:
                payload = {
                    "date": db_date_str,
        ...
        log_payload = {
            ...
            "status": "SUCCESS",
            "task_name": "[add_option_future_US_expiry_date] 미국 선옵 만기일 일정 생성",
        ...
        logs_ref.add({
            ...
            "updated_count": update_count,
            "skipped_count": skip_count,
            "message": f"미국 선옵만기일 일정 생성 실패 에러 로그: {error_msg}"
        })


if __name__ == "__main__":
    # 실행 시 이 배치 자신의 다음 실행 예정시간만 Firestore(crawler_schedules)에 기록
    update_my_schedule(db, __file__, display_name="미국 선옵 만기일 일정 생성")
